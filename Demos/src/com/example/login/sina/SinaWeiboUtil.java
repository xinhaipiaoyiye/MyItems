...
public class SinaWeiboUtil {
	...
	class AuthDialogListener implements WeiboAuthListener {
		...
		public void onError(WeiboDialogError e) {
			...
			Toast.makeText(mContext, "分享失败，请检查网络连接。出错信息", 2000).show();
		}
	...
	public void show(long uid) {
		...
		api.show(uid, new RequestListener() {
			...
			public void onIOException(IOException e) {
				...
				// Util.showToast(mContext, "分享失败，请检查网络连接。出错信息：" +
				// e.getMessage());
				Toast.makeText(mContext, "分享失败，请检查网络连接。出错信息", 2000).show();
			}
			...
			public void onError(WeiboException e) {
				...
				LOG.showToast(mContext, "分享失败，请检查网络连接。出错信息：" + e.getMessage());
				// Toast.makeText(mContext, "分享失败，请检查网络连接。出错信息", 2000).show();
			}
			...
			public void onComplete(String json) {
				...
				try {
					...
					String userName = object.optString(Sinas.SINA_NAME);
	...
	public void update(String content, String lat, String lon) {
		...
		api.update(content, lat, lon, new RequestListener() {
			...
			public void onError(WeiboException e) {
				...
				if (e.getStatusCode() == 400) { // 相同内容短时间内不能分享
					...
					Toast.makeText(mContext,
							"分享失败，相同内容短时间内不能分享，请稍候再试吧。出错信息：" + e.getMessage(),
							2000).show();
				} else {
					// Util.showToast(mContext,
					// "分享失败，请检查网络连接。出错信息：" + e.getMessage());
					Toast.makeText(mContext,
							"分享失败，请检查网络连接。出错信息" + e.getMessage(), 2000).show();
				}
			}
			...
			public void onComplete(String str) {
				...
				Toast.makeText(mContext, "分享成功，去你绑定的新浪微博看看吧！", 2000).show();
			}
		});
	}
	...
	public void upload(String content, String file, String lat, String lon) {
		...
		System.out.println("怎么得到的：" + mAccessToken);
		SinaWeiboAPI api = new SinaWeiboAPI(mAccessToken);
		api.upload(content, file, lat, lon, new RequestListener() {
			...
			public void onComplete(String str) {
				...
				LoginActivity.getHandler().post(new Runnable() {
					@Override
					public void run() {
						LOG.showToast(mContext, "分享成功，去你绑定的新浪微博看看吧！");
					}
				});
			}
		});
